...
import { ImageBroken, ArrowUpRight } from "@phosphor-icons/react";
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-8">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="font-editorial text-2xl font-light leading-snug text-navy-900">{aircraft.name}</h3>
            {aircraft.tagline ? <p className="mt-1.5 text-sm text-slate-500">{aircraft.tagline}</p> : null}
          </div>
          <ArrowUpRight className="mt-1 h-4 w-4 shrink-0 text-slate-300" weight="thin" aria-hidden="true" />
        </div>
