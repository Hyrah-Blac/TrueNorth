...
export function SuccessState({
  ...
  return (
    <div className="flex flex-col items-center px-2 py-6 text-center sm:py-8">
      <span className="flex h-11 w-11 items-center justify-center rounded-full border border-sky-200 bg-sky-50 text-sky-600 sm:h-12 sm:w-12">
        <CheckCircle2 className="h-5 w-5 sm:h-6 sm:w-6" aria-hidden="true" />
      </span>
      <p className="spec-readout mt-4 text-[0.6875rem] uppercase tracking-widest2 text-sky-600">Confirmed</p>
      ...
      {primaryAction ? (
        <Button href={primaryAction.href} variant="primary" size="md" className="mt-6">
          {primaryAction.label}
        </Button>
      ) : null}
    </div>
