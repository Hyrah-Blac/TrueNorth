...
export function QuoteProgress() {
  ...
  return (
    <div className="animate-[fade-up_400ms_cubic-bezier(0.16,1,0.3,1)_both] border-t border-slate-100 bg-slate-50/60 px-6 py-2.5 motion-reduce:animate-none sm:px-10">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
        {CHECKLIST_ITEMS.map((item) => {
          ...
          return (
            <span
              key={item.key}
              className={`flex items-center gap-1 text-[9.5px] font-medium uppercase tracking-wide transition-colors duration-300 ease-editorial ${
            ...
            >
              ...
              <span
                className={`flex h-3 w-3 items-center justify-center rounded-full border transition-all duration-300 ease-editorial ${
                  known ? "border-sky-500 bg-sky-500 text-white" : "border-slate-300"
                }`}
              >
                {known ? <Check className="h-2 w-2" aria-hidden="true" /> : null}
              </span>
              {item.label}
            </span>
          );
        ...
        {isReady ? (
          <Link
            ...
            className="ml-auto animate-[fade-up_400ms_cubic-bezier(0.16,1,0.3,1)_both] rounded-xl border border-blue-500 bg-blue-500 px-2.5 py-1.5 text-[9.5px] font-medium uppercase tracking-wide text-white shadow-crisp transition-all duration-300 ease-editorial hover:-translate-y-0.5 hover:border-blue-700 hover:bg-blue-700 motion-reduce:animate-none"
          >
